export const recommendTask = (performerType, lowestTwoChapters) => {
    if (!lowestTwoChapters || lowestTwoChapters.length < 2) {
      console.log('No weak areas found for task recommendation.');
  ...
  console.log(`Performer Type: ${formattedPerformerType}`);
  console.log(`Task Order for Performer: ${taskOrder[formattedPerformerType]}`);

  // Ensure performerType is valid and tasks are assigned
  if (!taskOrder[formattedPerformerType]) {
    console.log(`Invalid performer type: ${formattedPerformerType}`);
  ...
  const selectedWeeklyTasks = taskOrder[formattedPerformerType].map(index => {
    if (index >= weeklyTasks.length) {
      console.log(`Invalid index ${index} for weekly tasks.`);
      return null;
    }
    console.log(`Selected task at index ${index}:`, weeklyTasks[index]);
