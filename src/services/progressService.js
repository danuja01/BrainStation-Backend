...
export const predictExamScore = async (studentData) => {
  ...
  // Function to get chapter descriptions from the Python service
  ...
  const getStudySessionRecommendations = (focusToStudyRatio) => {
    const recommendations = [];
  
    if (focusToStudyRatio > 0.75) {
      recommendations.push("Study for 50 minutes, followed by a 10-minute break.");
    } else if (focusToStudyRatio > 0.5) {
      recommendations.push("Study for 40 minutes, followed by a 15-minute break.");
    } else {
      recommendations.push("Study for 30 minutes, followed by a 10-minute break.");
    }
  
    recommendations.push("Make sure to take regular breaks to improve focus and retention.");
    recommendations.push("Break your study sessions into short chunks with frequent breaks for better focus.");
  ...
  export const predictScoresForAllModules = async (userId) => {
    try {
      ...
      const lowestChapters = []; // Overall lowest 2 chapters across all modules
      ...
      await Promise.all(
        enrolledModules.map(async (module) => {
          ...
          if (studentData.quizzes.length === 0) {
            ...
            lowestChapters.push(...lowestModuleChapters);
      ...
      const sortedLowestChapters = lowestChapters.sort((a, b) => a.score - b.score).slice(0, 2);
  
      const formattedLowestChapters = await Promise.all(
        sortedLowestChapters.map(async (chapter) => {
          const description = await getChapterDescriptions(chapter.chapter);
      ...
      if (completedModulePredictions.length > 0) {
        highestScoreModule = completedModulePredictions.reduce((prev, curr) => prev.predictedExamScore > curr.predictedExamScore ? prev : curr);
        lowestScoreModule = completedModulePredictions.reduce((prev, curr) => prev.predictedExamScore < curr.predictedExamScore ? prev : curr);
      }
      ...
      return {
        ...
        lowestChapters: formattedLowestChapters, // Overall lowest 2 chapters
