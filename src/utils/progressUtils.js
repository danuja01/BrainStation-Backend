...
export const getStudySessionRecommendation = (modulePredictions) => {
  ...
  const focusTimeRatio = modulePredictions.map(prediction => {
    ...
    if (ratio > 0.8) {
      return { moduleName: prediction.moduleName, recommendation: "Recommended: 45 minutes study followed by a 5-minute break." };
    } else if (ratio > 0.5) {
      return { moduleName: prediction.moduleName, recommendation: "Recommended: 30 minutes study followed by a 10-minute break." };
    } else {
      return { moduleName: prediction.moduleName, recommendation: "Recommended: 20 minutes study followed by a 15-minute break." };
    }
  });
