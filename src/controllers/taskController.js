...
export const getTaskRecommendationController = async (req, res) => {
    ...
    try {
      ...
      // Validate inputs
      if (!userId || !performer_type || !lowest_two_chapters || !Array.isArray(lowest_two_chapters) || lowest_two_chapters.length < 2) {
        console.log('Invalid or missing fields.');
        return res.status(400).json({ message: 'Missing or invalid required fields.' });
      }
  
      // Step 1: Remove any existing task set for the given userId
      let existingTaskSet = await Task.findOne({ student: userId });
      ...
      console.log('New task set created:', savedTask);
...
export const deleteSubtaskFromTaskController = async (req, res) => {
  ...
  export const getCompletedTasksCount = async (req, res) => {
    ...
    try {
      ...
      console.error('Error fetching completed tasks count:', error.message);
